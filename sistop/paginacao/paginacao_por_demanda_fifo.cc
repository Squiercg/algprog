		...
		switch(opcao){
			...
			{		
				...
				if(!tabelas_de_pagina[numeroprocesso][pagina].valido) {
				  for(i = 0; i<quantidade_de_quadros; i++){
				...
				/*Agora eu coloco que a pagina está alocada na memoria
                                  em qual quadro da memoria eu coloquei ela
                                  coloco que esse quadro foi ocupado
                                  mudo a falha de pagina*/
				if(quadro_livre != -1) {
				  tabelas_de_pagina[numeroprocesso][pagina].valido=1;
				  ...
				  quadro_livre = remove_da_fila();
				  for(i=0; i<quantidade_de_processos; i++) {
				    for(j=0; j<=pow(2,tamanho_p)-1; j++) {
				      if(tabelas_de_pagina[i][j].valido && tabelas_de_pagina[i][j].quadro==quadro_livre){
					tabelas_de_pagina[i][j].valido=false;
					i=quantidade_de_processos;
					break;
				      }
				    }
				  }
